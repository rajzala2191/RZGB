            ...
            <tbody className="divide-y divide-slate-800 text-slate-200">
              {bids.map(bid => (
                <tr key={bid.id} className="hover:bg-slate-800/50 transition-colors">
                  <td className="p-4 font-mono text-xs text-slate-500">{bid.tender?.id?.slice(0, 8) || bid.tender_id?.slice(0, 8)}</td>
                  <td className="p-4 font-semibold text-cyan-400">{bid.tender?.ghost_public_name || 'N/A'}</td>
