...
const SecurePDFViewer = ({ document, order }) => {
  ...
  return (
    <div
      className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm relative select-none"
      onContextMenu={handleContextMenu}
    >
      {/* Header */}
      <div className="p-4 border-b border-slate-200 bg-slate-50 flex justify-between items-center z-20 relative">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-emerald-50 rounded-lg text-emerald-500 border border-emerald-200">
            <ShieldCheck size={20} />
          </div>
          <div>
            ...
            <p className="text-xs text-slate-400 font-mono mt-0.5">ID: {document.id?.slice(0, 8).toUpperCase()}</p>
          </div>
        </div>
        <div className="flex gap-2">
          <button
            onClick={handleDownload}
            className="flex items-center gap-2 px-3 py-1.5 bg-white hover:bg-slate-50 text-slate-600 hover:text-slate-900 border border-slate-200 rounded-lg text-xs font-bold transition-all"
          >
      ...
      <div className="relative bg-slate-200 min-h-[500px] flex items-center justify-center overflow-hidden group">
        ...
        <div className="w-[95vw] sm:w-[80%] max-w-2xl bg-white shadow-xl relative z-10 flex flex-col p-8 sm:p-12 text-slate-900">
        ...
        >
          <div className="transform -rotate-45 text-slate-900/5 text-3xl sm:text-5xl font-black whitespace-nowrap select-none flex flex-col items-center justify-center w-full h-full gap-32">
      ...
      <div className="p-3 bg-slate-50 border-t border-slate-200 text-[10px] text-slate-400 font-mono flex justify-between items-center z-20 relative">
        ...
        <div className="flex items-center gap-2">
           ...
           <span className="text-emerald-600 font-bold">Valid Signature</span>
        </div>
      </div>
    </div>
  );
