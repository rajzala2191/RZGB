...
const ControlCentreLayout = ({ children }) => {
  ...
  return (
    <ErrorBoundary>
      <div className="flex overflow-hidden font-sans h-screen bg-slate-50 dark:bg-slate-950">
        <AnimatePresence>
          {sidebarOpen && (
            <motion.div
              initial={{ opacity: 0 }}
        ...
        >
          <div className="flex flex-col items-center justify-center py-4 border-b border-slate-200 dark:border-slate-800 shrink-0 gap-1">
            ...
            <span className="text-[10px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 hidden lg:block" title={PORTAL_LABELS.WORKSPACE}>
              {PORTAL_LABELS.WORKSPACE}
            </span>
          </div>
