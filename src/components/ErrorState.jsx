...
export const ErrorState = ({ title = 'Error', message, onRetry }) => (
  <div className="flex flex-col items-center justify-center p-20 h-full">
    <div className="bg-white border border-red-200 rounded-xl p-8 max-w-md w-full text-center shadow-sm">
      ...
      <p className="text-slate-500 mb-4">{message || 'An error occurred. Please try again.'}</p>
      <button
        onClick={onRetry || (() => window.location.reload())}
        className="w-full bg-orange-600 hover:bg-orange-500 active:scale-[0.98] text-white py-2 px-4 rounded-lg transition-all shadow-sm hover:shadow flex items-center justify-center gap-2"
      >
        <RefreshCw size={16} />
        Retry
      </button>
